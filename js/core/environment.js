/**************************************************
** GAME ENVIRONMENT CLASS (Text Overlay Fixed)
**************************************************/
...
var Environment = function(i, j, width, height) {
    ...
    this.restart = function(){
        ...
        this.walls = this.level.walls ? ArrayUtils.copy(this.level.walls) : [];
    };
    ...
    this.hasVisited = function(i, j) {
        if (this.removeWalls) { return true; }
        if (i < 0 || i >= this.i || j < 0 || j >= this.j) { return false; }
        return this.visible[i][j] === 1;
    };
    ...
    this.hasAWall = function(x, y){
        return ArrayUtils.search(this.walls, [x, y]) !== false;
    };

    this.hasClearLineOfSight = function(x1, y1, x2, y2) {
        if (x1 !== x2 && y1 !== y2) return false;
        if (y1 === y2) {
            let min = Math.min(x1, x2);
            let max = Math.max(x1, x2);
            for (let i = min + 1; i < max; i++) {
                if (this.hasAWall(i, y1)) return false; 
            }
        }
        else if (x1 === x2) {
            ...
            for (let j = min + 1; j < max; j++) {
                if (this.hasAWall(x1, j)) return false; 
            }
        }
        return true;
    };
    ...
    this.draw = function(ctx) {
        ...
        for (let i = 0; i < this.walls.length; i++) {
            ...
            ctx.drawImage(resources.images['wall_1'], wall[0]*this.width, wall[1]*this.height, this.width, this.height);
        }
    ...
    this.drawText = function(ctx, text, i, j, offset){
        ...
        var centerX = (i * this.width) + (this.width / 2);
        var positionY = (j * this.height) + offset;

        ctx.strokeText(text, centerX, positionY);
        ctx.fillText(text, centerX, positionY);
    }
